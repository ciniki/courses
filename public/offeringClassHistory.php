...
function ciniki_courses_offeringClassHistory($ciniki) {
    ...
    $rc = ciniki_courses_checkAccess($ciniki, $args['business_id'], 'ciniki.courses.offeringClassHistory');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( $args['field'] == 'class_date' ) {
        ...
        return ciniki_core_dbGetModuleHistoryReformat($ciniki, 'ciniki.courses', 'ciniki_course_history', 
            $args['business_id'], 'ciniki_course_offering_classes', $args['class_id'], $args['field'], 'date');
    }

    if( $args['field'] == 'start_time' || $args['field'] == 'end_time' ) {
        ciniki_core_loadMethod($ciniki, 'ciniki', 'core', 'private', 'dbGetModuleHistoryReformat');
        return ciniki_core_dbGetModuleHistoryReformat($ciniki, 'ciniki.courses', 'ciniki_course_history', 
            $args['business_id'], 'ciniki_course_offering_classes', $args['class_id'], $args['field'], 'time');
    }

    ciniki_core_loadMethod($ciniki, 'ciniki', 'core', 'private', 'dbGetModuleHistory');
    return ciniki_core_dbGetModuleHistory($ciniki, 'ciniki.courses', 'ciniki_course_history', 
        $args['business_id'], 'ciniki_course_offering_classes', $args['class_id'], $args['field']);
}
?>
