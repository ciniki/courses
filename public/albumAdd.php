...
// Description
// -----------
// This method will add a new photo album for the business.
//
// Arguments
// ---------
// api_key:
// auth_token:
// business_id:        The ID of the business to add the Photo Album to.
//
// Returns
// -------
//
function ciniki_courses_albumAdd(&$ciniki) {
    ...
    $rc = ciniki_courses_checkAccess($ciniki, $args['business_id'], 'ciniki.courses.albumAdd');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    $rc = ciniki_core_objectAdd($ciniki, $args['business_id'], 'ciniki.courses.album', $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['business_id'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.courses.album', 'object_id'=>$album_id));
