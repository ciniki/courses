...
function ciniki_courses_instructorImageAdd(&$ciniki) {
    ...
    $rc = ciniki_courses_checkAccess($ciniki, $args['tnid'], 'ciniki.courses.instructorImageAdd'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.courses.115', 'msg'=>'Unable to get a new UUID', 'err'=>$rc['err']));
    }
    $args['uuid'] = $rc['uuid'];
    ...
    return ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.courses.instructor_image', $args, 0x07);
}
?>
