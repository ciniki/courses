...
// Description
// -----------
// This method will add a new subcategory for the tenant.
//
// Arguments
// ---------
// api_key:
// auth_token:
// tnid:        The ID of the tenant to add the Subcategory to.
//
// Returns
// -------
//
function ciniki_courses_subcategoryAdd(&$ciniki) {
    //
    ...
    $rc = ciniki_courses_checkAccess($ciniki, $args['tnid'], 'ciniki.courses.subcategoryAdd');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( $rc['num_rows'] > 0 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.courses.298', 'msg'=>'You already have a subcategory with that name, please choose another.'));
    }
    ...
    $rc = ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.courses.subcategory', $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['tnid'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.courses.subcategory', 'object_id'=>$subcategory_id));
