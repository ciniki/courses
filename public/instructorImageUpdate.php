...
function ciniki_courses_instructorImageUpdate(&$ciniki) {
    ...
    $rc = ciniki_courses_checkAccess($ciniki, $args['business_id'], 'ciniki.courses.instructorImageUpdate'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
	...
	//
	$strsql = "SELECT uuid, image_id FROM ciniki_course_instructor_images "
	...
	//
	$strsql = "UPDATE ciniki_course_instructor_images SET last_updated = UTC_TIMESTAMP()";
	...
	foreach($changelog_fields as $field) {
		if( isset($args[$field]) && $args[$field] != '' ) {
			...
			$rc = ciniki_core_dbAddModuleHistory($ciniki, 'ciniki.courses', 
				...
				2, 'ciniki_course_instructor_images', $args['instructor_image_id'], $field, $args[$field]);
		}
	}
	...
	if( !isset($rc['num_affected_rows']) || $rc['num_affected_rows'] != 1 ) {
		ciniki_core_dbTransactionRollback($ciniki, 'ciniki.courses');
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'1253', 'msg'=>'Unable to update instructor image'));	
	}
	...
	if( isset($args['image_id']) && $item['image_id'] != $args['image_id']) {
		...
		$rc = ciniki_images_refClear($ciniki, $args['business_id'], array(
			...
			'object_id'=>$args['instructor_image_id']));
		if( $rc['stat'] == 'fail' ) {
			ciniki_core_dbTransactionRollback($ciniki, 'ciniki.courses');
			return $rc;
		}
	...
	$ciniki['syncqueue'][] = array('push'=>'ciniki.courses.instructor_image', 
		'args'=>array('id'=>$args['instructor_image_id']));
