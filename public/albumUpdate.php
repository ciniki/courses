...
// -------
//
function ciniki_courses_albumUpdate(&$ciniki) {
    //
    ...
    $rc = ciniki_courses_checkAccess($ciniki, $args['business_id'], 'ciniki.courses.albumUpdate');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( isset($args['name']) ) {
        ...
        if( $rc['num_rows'] > 0 ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.courses.87', 'msg'=>'You already have an photo album with this name, please choose another.'));
        }
    }
    ...
    //
    // Update the Photo Album in the database
    //
    ciniki_core_loadMethod($ciniki, 'ciniki', 'core', 'private', 'objectUpdate');
    $rc = ciniki_core_objectUpdate($ciniki, $args['business_id'], 'ciniki.courses.album', $args['album_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['business_id'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.courses.album', 'object_id'=>$args['album_id']));
