...
function ciniki_courses_offerings() {
    ...
    this.menu = new M.panel('Course Offerings',
        ...
        'mc', 'xlarge narrowaside', 'sectioned', 'ciniki.courses.offerings.menu');
    this.menu.data = {};
    this.menu.sections = {
        ...
        'search':{'label':'', 'type':'livesearchgrid', 'livesearchcols':1,
            ...
            'hint':'Search course',
            'noData':'No course found',
            },
        'current':{'label':'Current Offerings', 'type':'simplegrid', 'num_cols':5,
            'visible':function() { return (M.ciniki_courses_offerings.menu.sections._tabs.selected == 'current' ? 'yes' : 'hidden'); },
            'headerValues':['Code', 'Course', 'Category', 'Start', 'End', 'Registrations'],
            'cellClasses':['', '', '', '', '', 'alignright'],
            'sortable':'yes', 
            'sortTypes':['text', 'text', 'text', 'date', 'date', 'number'],
            'noData':'No current offerings',
            },
        'upcoming':{'label':'Upcoming Offerings', 'type':'simplegrid', 'num_cols':5,
        ...
        'past':{'label':'Previous Offerings', 'type':'simplegrid', 'num_cols':5,
            'visible':function() { return (M.ciniki_courses_offerings.menu.sections._tabs.selected == 'past' ? 'yes' : 'hidden'); },
            'headerValues':['Code', 'Course', 'Category', 'Start', 'End', 'Registrations'],
            'cellClasses':['', '', '', '', '', 'alignright'],
            'sortable':'yes', 
    ...
    this.menu.liveSearchCb = function(s, i, v) {
        if( s == 'search' && v != '' ) {
            M.api.getJSONBgCb('ciniki.courses.offeringSearch', {'tnid':M.curTenantID, 'start_needle':v, 'limit':'25'}, function(rsp) {
                M.ciniki_courses_offerings.menu.liveSearchShow('search',null,M.gE(M.ciniki_courses_offerings.menu.panelUID + '_' + s), rsp.offerings);
                });
        }
    }
    this.menu.liveSearchResultValue = function(s, f, i, j, d) {
        return '<span class="maintext">' + (d.code != '' ? d.code + ' - ' : '') + d.course_name + (d.name != '' ? ' <span class="subdue">' + d.name + '</span>' : '') + '</span><span class="subtext">' + d.condensed_date + '</span>';
    }
    ...
    this.menu.cellValue = function(s, i, j, d) {
        switch(j) {
            case 0: return d.offering_code;
            case 1: return d.course_name;
            case 2: return d.offering_name;
            case 3: return d.start_date;
            case 4: return d.end_date;
            case 5: return d.num_registrations + '/' + d.num_seats;
        }
/*        if( j == 0 ) {
            var odate = '';
    ...
    this.menu.open = function(cb) {
        ...
        M.api.getJSONCb('ciniki.courses.offeringListV1', {'tnid':M.curTenantID, 'upcoming':'yes', 'current':'yes', 'past':'yes', 'files':'yes', 'limit':'26'}, function(rsp) {
            if( rsp.stat != 'ok' ) {
                M.api.err(rsp);
                return false;
            }
    ...
    //
    // The offering panel will show the information for a course offering.  Course
    ...
    this.offering.sections = {
        ...
        '_images':{'label':'', 'type':'simplegrid', 'num_cols':1,
            ...
            'addFn':'M.ciniki_courses_offerings.image.open(\'M.ciniki_courses_offerings.showOffering();\',0,M.ciniki_courses_offerings.offering.course_id);',
            },
...
//              },
    ...
    this.offering.thumbFn = function(s, i, d) {
        return 'M.ciniki_courses_offerings.image.open(\'M.ciniki_courses_offerings.showOffering();\',' + d.id + ',M.ciniki_courses_offerings.course_id);';
    };
    this.offering.cellValue = function(s, i, j, d) {
        if( s == 'classes' && j == 0 ) { 
            return '<span class="maintext">' + d.class_date + '</span><span class="subdue"> ' + d.start_time + ' - ' + d.end_time + '</span>';
        }
        if( s == 'prices' ) {
            switch(j) {
                case 0: return d.name + (d.available_to_text!=''?' <span class="subdue">(' + d.available_to_text + ')</span>':'');
...
//          }
    ...
    this.offering.rowFn = function(s, i, d) {
        if( s == 'classes' ) {
            return 'M.startApp(\'ciniki.courses.classes\',null,\'M.ciniki_courses_offerings.showOffering();\',\'mc\',{\'class_id\':\'' + d.id + '\'});';
        }
        if( s == 'prices' ) {
            return 'M.startApp(\'ciniki.courses.prices\',null,\'M.ciniki_courses_offerings.showOffering();\',\'mc\',{\'price_id\':\'' + d.id + '\'});';
        }
        if( s == 'instructors' ) {
            return 'M.startApp(\'ciniki.courses.instructors\',null,\'M.ciniki_courses_offerings.showOffering();\',\'mc\',{\'offering_instructor_id\':\'' + d.id + '\'});';
        }
//        if( s == 'files' ) {
//            return 'M.startApp(\'ciniki.courses.files\',null,\'M.ciniki_courses_offerings.showOffering();\',\'mc\',{\'file_id\':\'' + d.file_id + '\',\'offering_file_id\':\'' + d.id + '\'});';
//        }
        if( s == 'customers' ) {
            return 'M.startApp(\'ciniki.courses.customers\',null,\'M.ciniki_courses_offerings.showOffering();\',\'mc\',{\'offering_customer_id\':\'' + d.id + '\'});';
        }
    };
    ...
    this.edit.sections = {
        ...
        'offering':{'label':'Session', 'aside':'yes', 'fields':{
            ...
            'days':{'label':'Days', 'active':'no', 'none':'yes', 'type':'multiselect', 'options':['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']},
    ...
    //
    this.image = new M.panel('Image', 'ciniki_courses_offerings', 'image', 'mc', 'medium mediumaside', 'sectioned', 'ciniki.courses.main.image');
    this.image.data = null;
    this.image.course_id = 0;
    this.image.course_image_id = 0;
    this.image.sections = {
        '_image_id':{'label':'Image', 'type':'imageform', 'aside':'yes', 'fields':{
            'image_id':{'label':'', 'type':'image_id', 'hidelabel':'yes', 'controls':'all', 'history':'no',
                'addDropImage':function(iid) {
                    M.ciniki_courses_offerings.image.setFieldValue('image_id', iid);
                    return true;
                    },
                'addDropImageRefresh':'',
             },
        ...
        '_buttons':{'label':'', 'buttons':{
            'save':{'label':'Save', 'fn':'M.ciniki_courses_offerings.image.save();'},
            'delete':{'label':'Delete', 
                'visible':function() {return M.ciniki_courses_offerings.image.course_image_id > 0 ? 'yes' : 'no'; },
                'fn':'M.ciniki_courses_offerings.image.remove();'},
            }},
        };
    ...
    this.image.open = function(cb, cid, course_id) {
        ...
        M.api.getJSONCb('ciniki.courses.imageGet', {'tnid':M.curTenantID, 'course_image_id':this.course_image_id}, function(rsp) {
            ...
            var p = M.ciniki_courses_offerings.image;
            p.data = rsp.image;
            p.refresh();
            p.show(cb);
        });
    ...
    this.image.save = function(cb) {
        if( cb == null ) { cb = 'M.ciniki_courses_offerings.image.close();'; }
        ...
        if( this.course_image_id > 0 ) {
            ...
            M.api.postJSONCb('ciniki.courses.imageAdd', {'tnid':M.curTenantID, 'course_id':this.course_id}, c, function(rsp) {
                ...
                M.ciniki_courses_offerings.image.course_image_id = rsp.id;
                eval(cb);
            });
        }
    }
    ...
    this.start = function(cb, appPrefix, aG) {
        ...
        if( M.modFlagOn('ciniki.courses', 0x40) ) { 
            this.menu.sections.current.num_cols = 6;
            this.menu.sections.upcoming.num_cols = 6;
            this.menu.sections.past.num_cols = 6;
        } else {
            this.menu.sections.current.num_cols = 5;
            this.menu.sections.upcoming.num_cols = 5;
            this.menu.sections.past.num_cols = 5;
        }
    ...
    this.showOffering = function(cb, oid) {
        ...
        var rsp = M.api.getJSONCb('ciniki.courses.offeringGet',
            ...
            'classes':'yes', 'instructors':inst, 'files':files, 'prices':'yes', 'images':'yes', 'registrations':reg}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
