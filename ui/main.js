...
function ciniki_courses_main() {
    ...
    this.menutabs = {'label':'', 'visible':'no'};
...
//    this.offerings.addButton('add', 'Add', 'M.ciniki_courses_main.offering.open(\'M.ciniki_courses_main.offerings.open();\',0,0);');
    this.offerings.addButton('tools', 'Tools', 'M.startApp(\'ciniki.courses.tools\',null,\'M.ciniki_courses_main.offerings.open();\');');
    ...
    this.offering.sections = {
        ...
        'general':{'label':'Session', 'aside':'yes', 'fields':{
            ...
            'sequence':{'label':'Order', 'type':'text', 'size':'small',
                'visible':function() { return M.modFlagSet('ciniki.courses', 0x100000);},
                },
        ...
        'instructors':{'label':'Instructors', 'type':'simplegrid', 'num_cols':1, 'aside':'yes',
            'cellClasses':['multiline', 'alignright'],
            'noData':'No instructors added',
            'menu':{
                'add':{
                    'label':'Add Instructor',
                    'fn':'M.ciniki_courses_main.offering.save("M.ciniki_courses_main.oinstructor.open(\'M.ciniki_courses_main.offering.open();\',0,M.ciniki_courses_main.offering.offering_id,M.ciniki_courses_main.offering.course_id);");',
                    }},
            },
        '_tabs':{'label':'', 'type':'paneltabs', 'selected':'registrations', 'tabs':{
            ...
            'sponsorships':{'label':'Sponsorships', 
                'visible':function() { return M.modFlagSet('ciniki.sponsors', 0x10);},
                'fn':'M.ciniki_courses_main.offering.switchTab("sponsorships");',
                },
            'expenses':{'label':'Expenses', 
                'visible':function() { return M.modFlagSet('ciniki.courses', 0x0400);},
                'fn':'M.ciniki_courses_main.offering.switchTab("expenses");',
                },
//            'nqueue':{'label':'Queue', 
//                'visible':function() { return M.modFlagSet('ciniki.courses', 0x080000);},
...
//                },
        ...
        'sponsorships':{'label':'Sponsorships', 'type':'simplegrid', 'num_cols':3,
            'visible':function() { return M.ciniki_courses_main.offering.sections._tabs.selected == 'sponsorships' ? 'yes' : 'hidden';},
            'headerValues':['Date', 'Customer', 'Amount'],
            'cellClasses':['', '', ''],
            'noData':'No sponsorships',
//            'addTxt':'Add Sponsorships',
//            'addTopFn':'M.startApp(\'ciniki.sponsors.settings\',null,\'M.ciniki_courses_main.offering.open();\',\'mc\',{\'sponsor_id\':0,\'object\':\'ciniki.courses.offering\',\'object_id\':M.ciniki_courses_main.offering.offering_id});',
            },
    ...
    this.offering.switchTab = function(t) {
        ...
        this.showHideSections(['classes', 'messages', 'registrations', '_synopsis', '_content', '_materials', '_paid', '_paid_dt', 'files', 'images', '_images', 'notifications', 'nqueue', 'sponsorships', 'sponsorshippackages', 'expenses']);
    }
...
//                case 1: return M.btn('Remove', 'M.ciniki_courses_main.offering.removeInstructor(\'' + i + '\');');
    ...
    this.offering.footerValue = function(s, i, d) {
        if( s == 'sponsorships' ) {
            if( i == 2 ) {
    ...
    this.offering.rowFn = function(s, i, d) {
        ...
        if( s == 'instructors' ) {
            return 'M.ciniki_courses_main.offering.save("M.ciniki_courses_main.oinstructor.open(\'M.ciniki_courses_main.offering.open();\',\'' + d.id + '\');");';
        }
        if( s == 'registrations' ) {
        ...
        if( s == 'expenses' ) {
            return 'event.stopPropagation();M.startApp(\'ciniki.sapos.main\',null,\'M.ciniki_courses_main.offering.open();\',\'mc\',{\'expense_id\':\'' + d.id + '\'});';
        }
        return '';
    }
...
/*
    ...
    this.oinstructor.sections = {
        ...
        '_additional_info':{'label':'Addition Info', 'fields':{
...
*/
    //
    // The panel to edit Course Offering Instructor
    //
    this.oinstructor = new M.panel('Course Session Instructor', 'ciniki_courses_main', 'oinstructor', 'mc', 'medium', 'sectioned', 'ciniki.courses.main.oinstructor');
    this.oinstructor.data = null;
    this.oinstructor.offeringinstructor_id = 0;
    this.oinstructor.nplist = [];
    this.oinstructor.sections = {
        'general':{'label':'Session Instructor', 'fields':{
            'instructor_id':{'label':'Instructor', 'type':'select', 'options':{}, 'complex_options':{'value':'id', 'name':'name'}},
            'prefix':{'label':'Prefix', 'type':'text'},
            'suffix':{'label':'Suffix', 'type':'text'},
            }},
        '_buttons':{'label':'', 'buttons':{
            'save':{'label':'Save', 'fn':'M.ciniki_courses_main.oinstructor.save();'},
            'delete':{'label':'Delete', 
                'visible':function() {return M.ciniki_courses_main.oinstructor.offeringinstructor_id > 0 ? 'yes' : 'no'; },
                'fn':'M.ciniki_courses_main.oinstructor.remove();'},
            }},
        };
    this.oinstructor.fieldValue = function(s, i, d) { return this.data[i]; }
    this.oinstructor.fieldHistoryArgs = function(s, i) {
        return {'method':'ciniki.courses.offeringInstructorHistory', 'args':{'tnid':M.curTenantID, 'offeringinstructor_id':this.offeringinstructor_id, 'field':i}};
    }
    this.oinstructor.open = function(cb, oid, of_id, cid, list) {
        if( oid != null ) { this.offeringinstructor_id = oid; }
        if( list != null ) { this.nplist = list; }
        M.api.getJSONCb('ciniki.courses.offeringInstructorGet', {'tnid':M.curTenantID, 'offeringinstructor_id':this.offeringinstructor_id}, function(rsp) {
            if( rsp.stat != 'ok' ) {
                M.api.err(rsp);
                return false;
            }
            ...
            p.sections.general.fields.instructor_id.options = rsp.instructors;
            if( oid != null && oid == 0 && p.data.offering_id == 0 && of_id != null ) {
                p.data.offering_id = of_id;
            }
            if( oid != null && oid == 0 && p.data.course_id == 0 && cid != null ) {
                p.data.course_id = cid;
            }
            p.refresh();
            p.show(cb);
    ...
    this.oinstructor.save = function(cb) {
        if( cb == null ) { cb = 'M.ciniki_courses_main.oinstructor.close();'; }
        if( !this.checkForm() ) { return false; }
        if( this.offeringinstructor_id > 0 ) {
            var c = this.serializeForm('no');
            if( c != '' ) {
                M.api.postJSONCb('ciniki.courses.offeringInstructorUpdate', {'tnid':M.curTenantID, 'offeringinstructor_id':this.offeringinstructor_id}, c, function(rsp) {
                    if( rsp.stat != 'ok' ) {
                        M.api.err(rsp);
                        return false;
                    }
            ...
            var c = this.serializeForm('yes');
            c += '&offering_id=' + this.data.offering_id;
            c += '&course_id=' + this.data.course_id;
            M.api.postJSONCb('ciniki.courses.offeringInstructorAdd', {'tnid':M.curTenantID}, c, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
                M.ciniki_courses_main.oinstructor.offeringinstructor_id = rsp.id;
                eval(cb);
            });
        }
    }
    this.oinstructor.remove = function() {
        M.confirm('Are you sure you want to remove this instructor from the session?', null, function(rsp) {
            M.api.getJSONCb('ciniki.courses.offeringInstructorDelete', {'tnid':M.curTenantID, 'offeringinstructor_id':M.ciniki_courses_main.oinstructor.offeringinstructor_id}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
    ...
    this.oinstructor.nextButtonFn = function() {
        if( this.nplist != null && this.nplist.indexOf('' + this.offeringinstructor_id) < (this.nplist.length - 1) ) {
            return 'M.ciniki_courses_main.oinstructor.save(\'M.ciniki_courses_main.oinstructor.open(null,' + this.nplist[this.nplist.indexOf('' + this.offeringinstructor_id) + 1] + ');\');';
        }
        return null;
    }
    this.oinstructor.prevButtonFn = function() {
        if( this.nplist != null && this.nplist.indexOf('' + this.offeringinstructor_id) > 0 ) {
            return 'M.ciniki_courses_main.oinstructor.save(\'M.ciniki_courses_main.oinstructor.open(null,' + this.nplist[this.nplist.indexOf('' + this.offeringinstructor_id) - 1] + ');\');';
        }
        return null;
    }
    this.oinstructor.addButton('save', 'Save', 'M.ciniki_courses_main.oinstructor.save();');
    this.oinstructor.addClose('Cancel');
    this.oinstructor.addButton('next', 'Next');
    this.oinstructor.addLeftButton('prev', 'Prev');
    ...
    this.courses.sections = {
        '_tabs':this.menutabs,
        'org_categories':{'label':'Categories', 'type':'simplegrid', 'num_cols':1, 'aside':'yes', 
            'selected':0,
            'noData':'No Categories',
            'visible':function() { return M.modFlagSet('ciniki.courses', 0x100000);},
            'editFn':function(s,i,d) {
                if( d != null ) {
                    return 'M.ciniki_courses_main.category.open(\'M.ciniki_courses_main.courses.open();\',\'' + d.id + '\');';
                }
                return '';
                },
            'menu':{
                'add':{
                    'label':'Add Category',
                    'fn':'M.ciniki_courses_main.category.open("M.ciniki_courses_main.courses.open();", 0);',
                    },
                },
            'seqDrop':function(e,from,to){
                M.api.getJSONCb('ciniki.courses.categoryUpdate', {'tnid':M.curTenantID, 
                    'category_id':M.ciniki_courses_main.courses.data.org_categories[from].id,
                    'sequence':M.ciniki_courses_main.courses.data.org_categories[to].sequence,
                    }, function(rsp) {
                        if( rsp.stat != 'ok' ) {
                            M.api.err(rsp);
                            return false;
                        }
                        M.ciniki_courses_main.courses.open();
                    });
                },
            },
        'org_subcategories':{'label':'Subcategories', 'type':'simplegrid', 'num_cols':1, 'aside':'yes', 
            'selected':0,
            'noData':'No Subcategories',
            'visible':function() { return M.modFlagOn('ciniki.courses', 0x100000) && M.ciniki_courses_main.courses.sections.org_categories.selected > 0 ? 'yes' : 'no';},
            'editFn':function(s,i,d) {
                if( d != null ) {
                    return 'M.ciniki_courses_main.subcategory.open(\'M.ciniki_courses_main.courses.open();\',\'' + d.id + '\');';
                }
                return '';
                },
            'menu':{
                'add':{
                    ...
                    'fn':'M.ciniki_courses_main.subcategory.open("M.ciniki_courses_main.courses.open();", 0, M.ciniki_courses_main.courses.sections.org_categories.selected);',
                    },
                },
            'seqDrop':function(e,from,to){
                M.api.getJSONCb('ciniki.courses.subcategoryUpdate', {'tnid':M.curTenantID, 
                    'subcategory_id':M.ciniki_courses_main.courses.data.org_subcategories[from].id,
                    'sequence':M.ciniki_courses_main.courses.data.org_subcategories[to].sequence,
                    }, function(rsp) {
                        if( rsp.stat != 'ok' ) {
                            M.api.err(rsp);
                            return false;
        ...
        'statuses':{'label':'Status', 'type':'simplegrid', 'num_cols':1, 'aside':'yes', 'selected':'30',
            'visible':function() { return M.modFlagOn('ciniki.courses', 0x100000) ? 'no' : 'yes';},
            },
    ...
    this.courses.rowClass = function(s, i, d) {
        if( s == 'org_categories' && d.id == this.sections.org_categories.selected ) {
            return 'highlight';
        }
        if( s == 'org_subcategories' && d.id == this.sections.org_subcategories.selected ) {
            return 'highlight';
        }
        if( s == 'statuses' || s == 'levels' || s == 'types' || s == 'categories' || s == 'mediums' || s == 'ages' ) {
            if( this.sections[s].selected == d.value ) {
    ...
    this.courses.rowFn = function(s, i, d) {
        if( s == 'org_categories' ) {
            return 'M.ciniki_courses_main.courses.selectOrgCategory(' + d.id + ');';
        }
        if( s == 'org_subcategories' ) {
            return 'M.ciniki_courses_main.courses.selectOrgSubcategory(' + d.id + ');';
        }
        if( s == 'statuses' || s == 'levels' || s == 'types' || s == 'categories' || s == 'mediums' || s == 'ages' ) {
    ...
    this.courses.selectOrgCategory = function(i) {
        this.sections.org_categories.selected = i;
        this.sections.org_subcategories.selected = 0;
        this.open();
    }
    this.courses.selectOrgSubcategory = function(i) {
        this.sections.org_subcategories.selected = i;
        this.open();
    }
    ...
    this.courses.open = function(cb) {
        ...
        if( M.modFlagOn('ciniki.courses', 0x100000) ) {
            ...
            args['subcategory_id'] = this.sections.org_subcategories.selected;
        }
    ...
    this.courses.addButton('tools', 'Tools', 'M.startApp(\'ciniki.courses.tools\',null,\'M.ciniki_courses_main.courses.open();\');');
...
//            'addFn':'M.ciniki_courses_main.course.save("M.ciniki_courses_main.offering.open(\'M.ciniki_courses_main.course.open();\',0,M.ciniki_courses_main.course.course_id);");',
    ...
    this.course.open = function(cb, cid, list, form_sub_id) {
        ...
        M.api.getJSONCb('ciniki.courses.courseGet', {'tnid':M.curTenantID, 'course_id':this.course_id, 'form_submission_id':this.form_submission_id, 'subcategory_id':M.ciniki_courses_main.courses.sections.org_subcategories.selected}, function(rsp) {
            if( rsp.stat != 'ok' ) {
                M.api.err(rsp);
                return false;
            }
            ...
            p.data = rsp.course;
            p.sections.general.fields.subcategory_id.options = [];
            if( rsp.org_subcategories != null ) {
                p.sections.general.fields.subcategory_id.options = rsp.org_subcategories;
            }
    ...
    this.instructors.cellValue = function(s, i, j, d) {
        ...
        if( s == 'instructors' && M.modFlagOn('ciniki.courses', 0x0400) ) {
            switch(j) {
                case 0: return d.name;
                case 1: return d.status_text;
                case 2: return d.rating;
                case 3: return d.hourly_rate_display;
                case 4: return d.num_offerings;
                case 5: return d.last_offering;
            }
        } else if( s == 'instructors' ) {
            switch(j) {
                case 0: return d.name;
                case 1: return d.status_text;
    ...
    this.instructors.rowClass = function(s, i, d) {
        ...
        if( (s == 'levels' || s == 'mediums' || s == 'types') && this.sections[s].selected == d.value ) {
            return 'highlight';
        }
        return '';
    }
    ...
    this.instructors.open = function(cb) {
        M.api.getJSONCb('ciniki.courses.instructorList', {'tnid':M.curTenantID, 
            'status':this.sections.statuses.selected, 
            'level':this.sections.levels.selected, 
            'medium':this.sections.mediums.selected, 
            'type':this.sections.types.selected, 
            'stats':'yes'}, 
            function (rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
    ...
    this.instructors.addButton('tools', 'Tools', 'M.startApp(\'ciniki.courses.tools\',null,\'M.ciniki_courses_main.instructors.open();\');');
    ...
    this.instructor.sections = {
        ...
        'instructor':{'label':'Instructor Details', 'aside':'yes', 'fields':{
            ...
            'hourly_rate':{'label':'Hourly Rate', 'type':'text', 'size':'small',
                'visible':function() { return M.modFlagSet('ciniki.courses', 0x0400); },
                },
            }},
        ...
        '_tabs':{'label':'', 'type':'paneltabs', 'selected':'bio', 'tabs':{
            ...
            'notes':{'label':'Notes', 'fn':'M.ciniki_courses_main.instructor.switchTab("notes");'},
            }},
        ...
        '_notes':{'label':'Notes', 'type':'simpleform', 
            ...
            'fields':{
                'notes':{'label':'', 'type':'textarea', 'size':'xlarge', 'hidelabel':'yes'},
            }},
    ...
    this.instructor.switchTab = function(t) {
        this.sections._tabs.selected = t;
        this.refreshSection('_tabs');
        this.showHideSections(['_short_bio', '_full_bio', '_notes', 'offerings']);
    }
...
//                    + '{_formlink_} = Link to Form below<br/>'
    ...
    //
    this.category = new M.panel('Category', 'ciniki_courses_main', 'category', 'mc', 'medium mediumaside', 'sectioned', 'ciniki.courses.main.category');
    ...
    this.category.sections = {
        '_image_id':{'label':'Image', 'type':'imageform', 'aside':'yes', 'fields':{
            'image_id':{'label':'', 'type':'image_id', 'hidelabel':'yes', 'controls':'all', 'history':'no',
                'addDropImage':function(iid) {
                    M.ciniki_courses_main.category.setFieldValue('image_id', iid);
                    return true;
                    },
                'addDropImageRefresh':'',
             },
        }},
        'general':{'label':'Category', 'aside':'yes', 'fields':{
            'name':{'label':'Name', 'required':'yes', 'type':'text'},
        ...
        '_buttons':{'label':'', 'buttons':{
            'save':{'label':'Save', 'fn':'M.ciniki_courses_main.category.save();'},
            'delete':{'label':'Delete', 
                'visible':function() {return M.ciniki_courses_main.category.category_id > 0 ? 'yes' : 'no'; },
                'fn':'M.ciniki_courses_main.category.remove();'},
            }},
        };
    this.category.fieldValue = function(s, i, d) { return this.data[i]; }
    this.category.fieldHistoryArgs = function(s, i) {
        return {'method':'ciniki.courses.categoryHistory', 'args':{'tnid':M.curTenantID, 'category_id':this.category_id, 'field':i}};
    }
    this.category.open = function(cb, cid, list) {
        if( cid != null ) { this.category_id = cid; }
        if( list != null ) { this.nplist = list; }
        M.api.getJSONCb('ciniki.courses.categoryGet', {'tnid':M.curTenantID, 'category_id':this.category_id}, function(rsp) {
            if( rsp.stat != 'ok' ) {
                M.api.err(rsp);
                return false;
            }
            var p = M.ciniki_courses_main.category;
            p.data = rsp.category;
            p.refresh();
            p.show(cb);
        });
    ...
    this.category.save = function(cb) {
        if( cb == null ) { cb = 'M.ciniki_courses_main.category.close();'; }
        if( !this.checkForm() ) { return false; }
        if( this.category_id > 0 ) {
            var c = this.serializeForm('no');
            if( c != '' ) {
                M.api.postJSONCb('ciniki.courses.categoryUpdate', {'tnid':M.curTenantID, 'category_id':this.category_id}, c, function(rsp) {
                    if( rsp.stat != 'ok' ) {
                        M.api.err(rsp);
                        return false;
                    }
            ...
            var c = this.serializeForm('yes');
            M.api.postJSONCb('ciniki.courses.categoryAdd', {'tnid':M.curTenantID}, c, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
                M.ciniki_courses_main.category.category_id = rsp.id;
                eval(cb);
            });
        }
    }
    this.category.remove = function() {
        M.confirm('Are you sure you want to remove category?', null, function(rsp) {
            M.api.getJSONCb('ciniki.courses.categoryDelete', {'tnid':M.curTenantID, 'category_id':M.ciniki_courses_main.category.category_id}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
                M.ciniki_courses_main.category.close();
            });
        });
    }
    this.category.nextButtonFn = function() {
        if( this.nplist != null && this.nplist.indexOf('' + this.category_id) < (this.nplist.length - 1) ) {
            return 'M.ciniki_courses_main.category.save(\'M.ciniki_courses_main.category.open(null,' + this.nplist[this.nplist.indexOf('' + this.category_id) + 1] + ');\');';
        }
        return null;
    }
    ...
    this.category.addButton('save', 'Save', 'M.ciniki_courses_main.category.save();');
    ...
    this.subcategory.sections = {
        '_image_id':{'label':'Image', 'type':'imageform', 'aside':'yes', 'fields':{
            'image_id':{'label':'', 'type':'image_id', 'hidelabel':'yes', 'controls':'all', 'history':'no',
                'addDropImage':function(iid) {
                    M.ciniki_courses_main.subcategory.setFieldValue('image_id', iid);
                    return true;
                    },
                'addDropImageRefresh':'',
             },
        ...
        '_buttons':{'label':'', 'buttons':{
            'save':{'label':'Save', 'fn':'M.ciniki_courses_main.subcategory.save();'},
            'delete':{'label':'Delete', 
                'visible':function() {return M.ciniki_courses_main.subcategory.subcategory_id > 0 ? 'yes' : 'no'; },
                'fn':'M.ciniki_courses_main.subcategory.remove();'},
            }},
        };
    ...
    this.subcategory.open = function(cb, scid, cid, list) {
        ...
        M.api.getJSONCb('ciniki.courses.subcategoryGet', {'tnid':M.curTenantID, 'category_id':this.category_id, 'subcategory_id':this.subcategory_id}, function(rsp) {
            ...
            var p = M.ciniki_courses_main.subcategory;
            p.sections.general.fields.category_id.options = rsp.org_categories;
    ...
    this.subcategory.save = function(cb) {
        if( cb == null ) { cb = 'M.ciniki_courses_main.subcategory.close();'; }
        if( !this.checkForm() ) { return false; }
        if( this.subcategory_id > 0 ) {
            var c = this.serializeForm('no');
            if( c != '' ) {
                M.api.postJSONCb('ciniki.courses.subcategoryUpdate', {'tnid':M.curTenantID, 'subcategory_id':this.subcategory_id}, c, function(rsp) {
                    if( rsp.stat != 'ok' ) {
                        M.api.err(rsp);
                        return false;
                    }
            ...
            var c = this.serializeForm('yes');
            M.api.postJSONCb('ciniki.courses.subcategoryAdd', {'tnid':M.curTenantID}, c, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
                M.ciniki_courses_main.subcategory.subcategory_id = rsp.id;
                eval(cb);
            });
        }
    }
    this.subcategory.remove = function() {
        M.confirm('Are you sure you want to remove subcategory?', null, function(rsp) {
            M.api.getJSONCb('ciniki.courses.subcategoryDelete', {'tnid':M.curTenantID, 'subcategory_id':M.ciniki_courses_main.subcategory.subcategory_id}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
                M.ciniki_courses_main.subcategory.close();
            });
        });
    }
    this.subcategory.nextButtonFn = function() {
        if( this.nplist != null && this.nplist.indexOf('' + this.subcategory_id) < (this.nplist.length - 1) ) {
            return 'M.ciniki_courses_main.subcategory.save(\'M.ciniki_courses_main.subcategory.open(null,' + this.nplist[this.nplist.indexOf('' + this.subcategory_id) + 1] + ');\');';
        }
        return null;
    }
    ...
    this.subcategory.addButton('save', 'Save', 'M.ciniki_courses_main.subcategory.save();');
    ...
    this.tenantInit = function() {
        ...
        //
        // Setup the instructors list
        //
        if( M.modFlagOn('ciniki.courses', 0x0400) ) {
            this.instructors.sections.instructors.num_cols = 6;
            this.instructors.sections.instructors.headerValues = ['Name', 'Status', 'Rating', 'Hourly $', '# of Sessions', 'Last Session'];
            this.instructors.sections.instructors.headerClasses = ['', '', '', '', 'aligncenter', 'alignright',];
            this.instructors.sections.instructors.sortTypes = ['text', 'text', 'text', 'number', 'number', 'date'];
            this.instructors.sections.instructors.cellClasses = ['', '', '', '', 'aligncenter', 'alignright'];
        } else {
            this.instructors.sections.instructors.num_cols = 4;
            this.instructors.sections.instructors.headerValues = ['Name', 'Status', '# of Sessions', 'Last Session'];
            this.instructors.sections.instructors.headerClasses = ['', '', 'aligncenter', 'alignright',];
            this.instructors.sections.instructors.sortTypes = ['text', 'text', 'number', 'date'];
            this.instructors.sections.instructors.cellClasses = ['', '', 'aligncenter', 'alignright'];
        }
...
*/
        ...
        //
        // Set up drag drop reorder of courses and offerings
        //
        if( M.modFlagOn('ciniki.courses', 0x100000) ) {
            this.courses.sections.courses.seqDrop = function(e,from,to){
                M.api.getJSONCb('ciniki.courses.courseUpdate', {'tnid':M.curTenantID, 
                    'course_id':M.ciniki_courses_main.courses.data.courses[from].id,
                    'sequence':M.ciniki_courses_main.courses.data.courses[to].sequence,
                    }, function(rsp) {
                        if( rsp.stat != 'ok' ) {
                            M.api.err(rsp);
                            return false;
            ...
            this.course.sections.offerings.seqDrop = function(e,from,to){
                M.api.getJSONCb('ciniki.courses.offeringUpdate', {'tnid':M.curTenantID, 
                    'offering_id':M.ciniki_courses_main.course.data.offerings[from].id,
                    'sequence':M.ciniki_courses_main.course.data.offerings[to].sequence,
                    }, function(rsp) {
                        if( rsp.stat != 'ok' ) {
                            M.api.err(rsp);
                            return false;
                        }
                        M.ciniki_courses_main.course.open();
                    });
                };
        } else {
            this.courses.sections.courses.seqDrop = null;
            this.course.sections.offerings.seqDrop = null;
        }
        if( !M.modFlagOn('ciniki.courses', 0x100000) ) {
            ...
            if( M.modFlagOn('ciniki.courses', 0x02) ) {
                this.menutabs.tabs['instructors'] = {'label':'Instructors', 'fn':'M.ciniki_courses_main.switchTab("instructors");'};
            }
        } else {
            this.menutabs = {'label':'', 'type':'menutabs', 'visible':'no'};
        }
        this.courses.menutabs = this.menutabs;
        this.offerings.menutabs = this.menutabs;
        this.instructors.menutabs = this.menutabs;
    }
}
