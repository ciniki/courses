...
//              'valid_to':{'label':'Valid To', 'hint':'', 'type':'text'},
    ...
    this.showEdit = function(cb, pid, oid) {
        ...
        if( this.edit.price_id > 0 ) {
            ...
            M.api.getJSONCb('ciniki.courses.offeringPriceGet', {'tnid':M.curTenantID, 
    ...
    this.savePrice = function() {
        if( this.edit.price_id > 0 ) {
            ...
            M.api.postJSONCb('ciniki.courses.offeringPriceAdd', 
                {'tnid':M.curTenantID, 'offering_id':this.edit.offering_id}, c, function(rsp) {
                    if( rsp.stat != 'ok' ) {
                        M.api.err(rsp);
                        return false;
                    } 
    ...
    this.deletePrice = function() {
        M.confirm("Are you sure you want to remove this price?",null,function() {
            M.api.getJSONCb('ciniki.courses.offeringPriceDelete', 
                {'tnid':M.curTenantID, 
                'price_id':M.ciniki_courses_prices.edit.price_id}, function(rsp) {
                    if( rsp.stat != 'ok' ) {
                        M.api.err(rsp);
                        return false;
                    }
