...
// -------
//
function ciniki_courses_web_instructorList($ciniki, $settings, $business_id, $offering_id) {
	...
	$strsql = "SELECT ciniki_course_instructors.id, "
		...
		. "IF(ciniki_course_instructors.full_bio='', 'no', 'yes') AS isdetails, "
	...
	$rc = ciniki_core_dbHashQueryIDTree($ciniki, $strsql, 'ciniki.courses', array(
		array('container'=>'instructors', 'fname'=>'id', 
			'fields'=>array('id', 'name', 'permalink', 'image_id'=>'primary_image_id', 'short_bio', 'url', 'isdetails')),
		));
	if( $rc['stat'] != 'ok' ) {
		return $rc;
	}
