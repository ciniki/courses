...
error_log(print_r($courses_settings,true));
    ...
    if( !isset($rc['offering']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.courses.70', 'msg'=>'Unable to find requested class'));
    }
...
//  print "<pre>" . print_r($offering, true) . "</pre>";
    ...
    if( isset($offering['classes']) ) { 
        ...
        if( count($classes) > 0 ) {
            $first_date = array_shift($classes);
            $dates .= $first_date['class_date'];
            if( count($classes) > 0 ) {
                $last_date = array_pop($classes);
    ...
    $num_classes = count($offering['classes']);
    $date_width = 9;
    ...
    foreach($offering['registrations'] as $reg) {
        ...
        for($i = 0; $i < $num_classes; $i++) {
            $pdf->MultiCell($date_width, $lh, '', 1, 'L', $fill, 0);
        }
        $pdf->Ln();
